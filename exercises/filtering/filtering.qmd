...
```{r}
powerfive <- c("Pac-12 MBB", "Big Ten MBB", "SEC MBB", "Big 12 MBB", "ACC MBB")
players |>
  mutate(trueshooting = (PTS/(2*(FGA + (.44*FTA))))*100) |>
  arrange(desc(trueshooting)) |>
  filter(FGA > 135, Conference %in% powerfive)
```

**ANSWER** Who do you get as the first result, and what was his trueshooting percentage?
Moses Bol, 83%

I removed the minutes cutoff, added a field goal attempts cutoff and set it to >135 (the mean FGA of all players) and limited the players to only players in the power five conferences. This limits it to players playing at the highest possible collegiate level while adding a sample size cutoff to get a more accurate answer. After doing this, the top player is Braxton Meah, with a 72.1% TS. He is the most efficient player in the power five conference by this metric.
